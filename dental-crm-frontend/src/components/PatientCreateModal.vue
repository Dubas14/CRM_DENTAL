...
<template>
  <div
    v-if="isOpen"
    class="fixed inset-0 z-[60] flex items-center justify-center bg-text/20 dark:bg-bg/50 backdrop-blur-sm p-4"
  >
    <div
      class="bg-card rounded-xl shadow-sm shadow-black/10 dark:shadow-black/40 shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh]"
    >
      ...
      <div class="p-6 overflow-y-auto custom-scrollbar space-y-4">
        <div
          v-if="error"
          class="bg-red-900/30 border border-red-500/30 text-red-300 p-3 rounded text-sm"
        >
          {{ error }}
        </div>
        ...
        <form @submit.prevent="submit" class="space-y-4">
          ...
          <div class="pt-2 flex justify-end gap-3">
            <button
              type="button"
              @click="$emit('close')"
              class="px-4 py-2 text-text/70 hover:text-text"
            >
              Скасувати
            </button>
            <button
              type="submit"
              :disabled="loading"
              class="bg-emerald-600 text-text px-6 py-2 rounded hover:bg-emerald-500 disabled:opacity-50"
            >
