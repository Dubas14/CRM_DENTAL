...
const statuses = [
  ...
  { id: 'missing', label: 'Відсутній', color: 'bg-card/80 border-border text-text/70', fill: '#374151' },
...
<template>
  <div class="bg-card p-6 rounded-xl shadow-sm shadow-black/10 dark:shadow-black/40">
    ...
    <div v-else class="relative">
        ...
        <div class="flex gap-4">
          <div class="flex gap-1">
            <div v-for="t in quadrants.q1" :key="t" @click="selectedTooth = t"
              ...
              <svg width="40" height="50" viewBox="0 0 40 50" class="text-border transition-transform group-hover:scale-110">
                <path d="M5,15 Q5,0 20,0 Q35,0 35,15 L35,40 Q35,50 20,50 Q5,50 5,40 Z"
                      :fill="getFillColor(t)" stroke="currentColor" stroke-width="2" />
                <path v-if="teethMap[t] === 'missing'" d="M10,10 L30,40 M30,10 L10,40" stroke="currentColor" stroke-width="3"/>
              </svg>
            </div>
          </div>
          <div class="w-px bg-border mx-2"></div> <div class="flex gap-1">
      ...
      <div v-if="selectedTooth" class="fixed inset-0 bg-text/20 dark:bg-bg/50 flex items-center justify-center z-50" @click.self="selectedTooth = null">
        <div class="bg-card rounded-xl shadow-xl p-6 w-80 max-w-full">
          ...
          <button @click="selectedTooth = null" class="mt-4 w-full py-2 text-text/70 text-sm hover:text-text/60">
            Скасувати
          </button>
        </div>
      </div>
