  ...
  <div class="bg-card/60 border border-border rounded-xl p-4 space-y-3">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-lg font-semibold text-text">{{ appointment?.patient?.full_name || 'Запис' }}</p>
      </div>
      <span class="text-xs bg-card/80 px-2 py-1 rounded text-text/80">ID: {{ appointment.id }}</span>
    </div>
    ...
    <label class="space-y-1 block">
      ...
      <textarea v-model="comment" rows="2" class="w-full bg-bg border border-border/80 rounded-lg px-3 py-2 text-text" placeholder="Причина або інструкції для адміністратора"></textarea>
    </label>

    <div class="flex justify-end gap-3">
      <button class="px-4 py-2 text-text/70 hover:text-text" type="button" @click="$emit('close')">Закрити</button>
      <button
    ...
    <div v-if="suggestions.length" class="border-t border-border pt-3 mt-2 space-y-2">
      ...
      <div class="space-y-2">
        ...
        >
          ...
          <p class="text-xs text-text/60" v-if="entry.preferred_date">Бажана дата: {{ entry.preferred_date }}</p>
        </div>
      </div>
    </div>
  </div>
