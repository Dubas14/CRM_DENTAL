...
export function useCalendar() {
    ...
    try {
      ...
      if (isResourceView.value && resourceViewType.value === 'room') {
        missingRoomAppointmentsCount.value = appts.filter(
          (appt) =>
            (appt?.start_at || appt?.start) && (appt?.end_at || appt?.end) && appt?.room_id == null
        ).length
      } else {
      ...
      if (
        isResourceView.value &&
        resourceViewType.value === 'room' &&
        selectedRoomIds.value.length
      ) {
        const allowed = new Set(selectedRoomIds.value.map((id) => String(id)))
        mapped = mapped.filter(
          (event) =>
            event.resourceId === NO_ROOM_RESOURCE_ID ||
  ...
  const handleEventDrop = async (payload) => {
    ...
    try {
      ...
      const message =
        error?.response?.data?.message || error?.message || 'Помилка перенесення запису'
      toastError(message)
    }
  }
  ...
  const initialize = async () => {
    try {
      ...
      const message =
        initError?.response?.data?.message || initError?.message || 'Помилка ініціалізації'
      error.value = message
      toastError(message)
    } finally {
      loading.value = false
  ...
  watch(selectedDoctorId, async () => {
    if (
      selectedDoctorId.value &&
      !selectedDoctorIds.value.includes(String(selectedDoctorId.value))
    ) {
      selectedDoctorIds.value = [
        ...new Set([...selectedDoctorIds.value, String(selectedDoctorId.value)])
      ]
    }
  ...
  watch([selectedClinicId, () => doctors.value.length, () => filteredDoctors.value.length], () =>
