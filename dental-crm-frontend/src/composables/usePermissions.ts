...
export function usePermissions() {
  ...
  const canSeeClinics = computed(() => isSuperAdmin.value || isClinicAdmin.value)

  const canSeeDoctors = computed(() => isSuperAdmin.value || isClinicAdmin.value)
  ...
  const canManageRoles = computed(() => isSuperAdmin.value || isClinicAdmin.value)

  const canManageCatalog = computed(() => isSuperAdmin.value || isClinicAdmin.value)
